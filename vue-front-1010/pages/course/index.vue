              ...
              <li v-for="record in data.records" :key="record.id">
                <div class="cc-l-wrap">
                  <section class="course-img">
                    <img :src="record.cover" class="img-responsive" :alt="record.name" />
                    <div class="cc-mask">
                      <a :href="'/course/'+ record.id" title="开始学习" class="comm-btn cbtn-1">开始学习</a>
                    </div>
                  </section>
                  ...
                  <section class="mt10 hLh20 of">
                    <span v-if="Number(record.price) === 0" class="fr jgTag bg-green">
...
export default {
  ...
  methods: {
    initCourseFirst() {
      courseApi.pageAllCourse(1, 8, this.searchObj).then(response => {
    ...
    gotoPage(page) {
      courseApi.pageAllCourse(page, 8, this.searchObj).then(response => {
        this.data = response.data.data;
      });
    },
    searchOne(id, index) {
      this.oneIndex = index;
      this.twoIndex = -1;
      this.searchObj.sujectId = "";
      this.subSubjectList = [];
      this.searchObj.subjectParentId = id;
      this.gotoPage(1);
      for (let i = 0; i < this.subjectNestedList.length; i++) {
        var s = this.subjectNestedList[i];
        if (s.id == id) {
          this.subSubjectList = s.children;
